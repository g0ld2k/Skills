...
Create one unique message file with `mktemp` and register cleanup for normal
exit plus `HUP`, `INT`, and `TERM` before writing the approved message. A setup
or write failure stops the workflow and reports its command, status, and error.
